        ...
        <div className='rounded-lg shadow p-4 space-y-4' style={{background: 'var(--bg-card)', color: 'var(--text)', border: '1px solid var(--border)'}}>
            ...
            <div className='flex space-x-4 overflow-x-auto pb-2'>
                {data.shopOrders.shopOrderItems.map((item, index) => (
                    <div key={index} className='flex-shrink-0 w-40 border rounded-lg p-2' style={{background: 'var(--bg-card)', borderColor: 'var(--border)'}}>
...
<div className="mt-3 p-2 border rounded-lg text-sm gap-4" style={{background: 'var(--accent-soft)', color: 'var(--accent)', borderColor: 'var(--border)'}}>
...
<div className='text-right font-bold text-sm' style={{color: 'var(--accent)'}}> Total: ₹{data.shopOrders.subtotal}</div>
        </div>
    )
}
