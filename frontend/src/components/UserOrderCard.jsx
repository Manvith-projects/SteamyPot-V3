...
function UserOrderCard({ data }) {
    ...
    return (
        <div className='rounded-lg shadow p-4 space-y-4' style={{background: 'var(--bg-card)', color: 'var(--text)', border: '1px solid var(--border)'}}>
            <div className='flex justify-between border-b pb-2'>
                ...
                <div className='text-right'>
                    ...
                    <p className='font-medium text-blue-600'>{Array.isArray(data.shopOrders) && data.shopOrders[0]?.status}</p>
                    <button className='ml-2 px-3 py-1 rounded bg-[#ff4d2d] text-white text-xs font-semibold hover:bg-[#e64526]' onClick={() => dispatch && dispatch({ type: 'user/hideMyOrder', payload: data._id })}>Hide</button>
                </div>
            </div>

            {Array.isArray(data.shopOrders) && data.shopOrders.map((shopOrder, index) => (
                <div className='border rounded-lg p-3 space-y-3' key={shopOrder._id || index} style={{background: 'var(--bg-card)', borderColor: 'var(--border)'}}>
                    <p>{shopOrder?.shop?.name || 'Unknown Shop'}</p>

                    <div className='flex space-x-4 overflow-x-auto pb-2'>
                        {Array.isArray(shopOrder?.shopOrderItems) && shopOrder.shopOrderItems.map((item, idx) => (
                            <div key={item?.item?._id || idx} className='flex-shrink-0 w-40 border rounded-lg p-2' style={{background: 'var(--bg-card)', borderColor: 'var(--border)'}}>
                                {item?.item?.image ? (
                                    <img src={item.item.image} alt="" className='w-full h-24 object-cover rounded' />
                                ) : null}
                                <p className='text-sm font-semibold mt-1'>{item?.name || 'Unknown Item'}</p>
